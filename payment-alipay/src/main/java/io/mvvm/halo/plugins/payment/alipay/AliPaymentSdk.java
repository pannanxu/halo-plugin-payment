...
//                        config.setAlipayPublicKey(config.getAlipayPublicKey());
//                        config.setEncryptKey(config.getEncryptKey());
...
//                    model.setTotalAmount(Amount.of(request.getTotalFee()).toYuan());
...
//                    try {
//                        AlipayTradeQueryResponse response = alipayClientAtomicReference.get().execute(queryRequest);
//                        if (!response.isSuccess()) {
...
//                                .setTotalFee(Amount.ofYuan(response.getTotalAmount()).getTotal())
//                                .setActualFee(null == response.getBuyerPayAmount() ? 0 : Amount.ofYuan(response.getBuyerPayAmount()).getTotal())
...
//                    try {
//                        AlipayTradeCloseResponse response = alipayClientAtomicReference.get().execute(closeRequest);
//                        if (!response.isSuccess()) {
...
//                        model.setOutTradeNo(request.getOutTradeNo());
//                        model.setRefundAmount(request.getAmount().toYuan());
//                        model.setRefundReason(request.getRefundReason());
//                        model.setOutRequestNo(request.getOutTradeNo());
//                        model.setQueryOptions(List.of("refund_detail_item_list"));
//                        refundRequest.setBizModel(model);
//                        AlipayTradeRefundResponse response = alipayClientAtomicReference.get().execute(refundRequest);
//                        if (!response.isSuccess()) {
...
//                                .setTradeNo(response.getTradeNo())
//                                .setRefundAmount(Amount.ofYuan(response.getSendBackFee())));
//                    } catch (Exception e) {
...
//                        model.setTradeNo("123123");
//                        model.setOutRequestNo("HZ01RF001");
//                        refundQueryRequest.setBizModel(model);
//                        AlipayTradeFastpayRefundQueryResponse response = alipayClientAtomicReference.get().execute(refundQueryRequest);
//                        if (!response.isSuccess()) {
//                            return Mono.error(new RefundException(response.getSubCode(), response.getSubMsg()));
//                        }
